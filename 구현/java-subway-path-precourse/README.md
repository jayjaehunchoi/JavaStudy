...
## 기능 정리
#### 초기 설정
1. 역 등록 (교대역, 강남역, 역삼역, 남부터미널역, 양재역, 양재시민의숲역, 매봉역)
2. 노선 등록 (2호선, 3호선, 신분당선)
3. 구간 등록
  - 노선에 관계 없이 시간 구간 
  - 노선에 관계 없이 거리 구간
...
#### 선택
1. 입력값 받기 - ```1``` or ```2``` or ```B```
  - ```예외``` 1, 2, B가 입력되지 않을 시 선택 에러

2. ```1``` or ```2``` - 출발역 , 도착역 입력
  - ```예외``` 출발역, 도착역이 존재하지 않는 역일 시 다시 입력 요청
  - ```예외``` 출발역, 도착역 같으면 에러
  - ```예외``` 출발역, 도착역 구간 없으면 에러
