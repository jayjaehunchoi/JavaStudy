...
public class KakaoApi {
    ...
    public static String authenticateNumber(final User user) {
        ...
        System.out.println(user.getPhoneNumber() + " 에 카카오 인증번호 : " + authenticateNumber + "를 발송했습니다.");
    ...
    public static AuthResult authenticate(final User user, final String number) {
        ...
        if (Objects.isNull(user)) {
            System.out.println("카카오 인증실패! 존재하지 않는 유저");
            return AuthResult.FAIL;
        }

        if (!number.equals("12345")) {
            System.out.println("카카오 인증실패! 인증 번호 오류");
            return AuthResult.FAIL;
        }

        System.out.println("카카오 인증성공!");
